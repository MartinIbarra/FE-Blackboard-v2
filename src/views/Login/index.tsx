...
const Login: React.FC<{ rooms_list: RoomListI }> = ({ rooms_list }) => {
	...
	// useEffect(() => {
	//   console.log("room_list =>", rooms_list);
	// }, [rooms_list]);
			...
			<div className="flex p-4 justify-center w-full">
				<div className="flex flex-col gap-4 justify-center items-center w-96 h-full m-auto">
					<div className="flex gap-4 mb-4">
						<span className="cursor-pointer transition duration-300 hover:text-primary" onClick={() => setJoinRoom(true)}>
							Join room
						</span>
						<span className="cursor-pointer transition duration-300 hover:text-primary" onClick={() => setJoinRoom(false)}>
							Create room
						</span>
					</div>
					<div className="relative z-0 w-full mb-5 group text-white">
						<FloatingLabel className="text-white focus:border-[#DC5F00]" value={name} onChange={(e) => setName(e?.target.value)} variant="standard" label="Name" theme={theme} color={"default"} />
						{nameError.length > 0 && <span className="text-red-700"> {nameError}</span>}
					</div>
					{joinRoom ? (
						<div>{rooms_list.length > 0 && <RoomList validateName={validateName} inputName={name} rooms={rooms_list} />}</div>
					) : (
